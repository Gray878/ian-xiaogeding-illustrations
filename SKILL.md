...
使用已锁定模式的 QA 清单和素材集成 QA。静态图默认保存到 `assets/<article-slug>-illustrations/`，按 `01-topic-name.png` 编号；不要覆盖已有资产。视频项目使用 `video-projects/<slug>/`，最终视频保存到 `renders/<slug>.mp4`，并保留 `DESIGN.md`、字幕、时间表、`material-plan.json`、`ATTRIBUTION.md` 和揭示计划。

## 输出口径

策略输出简短而具体。生成后说明：生成数量、每张用途与模式、实际使用的本地素材数量、许可/署名处理、保存路径、最稳和可选的图；视频额外说明时长、分辨率、字幕来源、场景模式分布、素材覆盖率、贴图入场方式、B 档遮罩和校验结果。
