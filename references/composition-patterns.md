...
画法：左侧输入，中间小鸽钉或怪机器处理，右侧输出，橙色箭头表达主流向。

### 系统局部

适合：信息来源、过滤器、数据库、渲染器、agent 系统局部。

画法：只画 3-5 个核心模块，小鸽钉参与其中一个关键动作。
...
画法：一层层盒子，不要正式金字塔；小鸽钉在旁边搬运、插线、固定或搭建。

### 地图路线

适合：从想法到上线、用户路径、内容承接路径、学习路线。

画法：一条弯曲路径，少量节点，小鸽钉递送、接线或走路。
...
3. 让小鸽钉承担动作：不是站旁边，而是传信、搬运、接线、修补、固定节点、扶梯子、记录、把概念纸条送进某个怪装置。
...
- 送、递、背、扛、贴、钉、插、接、拧、补、分拣、记录、搭桥、修线、回收。
- 动作要服务核心意思，不要为了怪而怪。

## 反复刻规则

不要默认打开或复刻 `assets/examples/`。`assets/legacy-monochrome/` 是归档旧构图，默认不要读取；只有用户明确要求旧构图时才可参考其隐喻关系，绝不参考其黑白配色或旧角色造型。当前彩色风格以 `style-dna.md` 和 `color-system.md` 为准。
...
同类主题也要换新隐喻。例如“承接路径”不一定画路线，可以画小鸽钉把一封信塞进正确接口；“一鱼多吃”不一定画鱼，可以画小鸽钉把一个纸团分拣成几种用途。
